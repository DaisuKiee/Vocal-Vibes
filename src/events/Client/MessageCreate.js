...
async function getPrefix(guildId, client) {
    // If no MongoDB URL configured, just use default prefix
    ...
    try {
        ...
        // If DB query fails, fall back to default prefix
        return client.config.prefix;
    }
}
    ...
    async run(message) {
        ...
        try {
            ...
            if (this.client.discordLogger) {
                this.client.discordLogger.logCommand(ctx, commandName, args);
            }
            ...
            await message.channel.send({ content: `❌ Error: ${error.message || 'Unknown error'}` }).catch(() => { });
        }
    }
}
